  ...
  goto_programt::targett target)
{
  ...
  if(assigns.is_not_nil())
  {
    ...
    goto_programt assigns_havoc = assigns_cause.havoc_code(
      function_symbol.location, func_id, function_symbol.mode);
  ...
  assigns_clauset &assigns_clause)
{
  ...
  exprt cast_alias = not_exprt(binary_predicate_exprt(
    typecast_exprt(
      typecast_exprt(not_exprt(alias_expr), signed_long_int_type()),
      signed_long_int_type()),
    ID_notequal,
    constant_exprt(irep_idt(dstringt("0")), signed_long_int_type())));
  ...
  if(call.function().id() == ID_symbol)
  {
    ...
    else // Called function has assigns clause
    {
      ...
      assigns_clauset called_assigns_clause(
        called_assigns, *this, func_id, log);
      exprt compat =
...
bool code_contractst::check_for_looped_mallocs(const goto_programt &program)
{
  ...
  for(auto goto_entry : back_gotos)
  {
    for(const auto &t : goto_entry.targets)
    {
      ...
      {
        if(
          malloc_entry.location_number >= t->location_number &&
...
bool code_contractst::add_pointer_checks(const std::string &func_name)
{
  ...
  goto_programt mark_dead =
    assigns.dead_stmts(f_sym.location, func_name, f_sym.mode);
  ...
  for(; ins_it != program.instructions.end(); std::advance(ins_it, 1))
  {
    if(ins_it->is_decl())
    {
      freely_assignable_symbols.insert(
        ins_it->get_decl().symbol().get_identifier());

      assigns_clause_targett *new_target =
        assigns.add_target(ins_it->get_decl().symbol());
      goto_programt &pointer_capture = new_target->get_init_block();
...
assigns_clause_scalar_targett::assigns_clause_scalar_targett(
  const exprt &obj_ptr,
  const code_contractst &contr,
  messaget &log_param,
  const irep_idt &func_id)
  : assigns_clause_targett(Scalar, pointer_for(obj_ptr), contr, log_param),
    local_standin_var(typet())
{
  ...
  symbolt standin_symbol = contract.new_tmp_symbol(
    obj_pointer.type(), f_sym.location, func_id, f_sym.mode);
...
goto_programt
assigns_clause_scalar_targett::havoc_code(source_locationt loc) const
{
...
assigns_clause_struct_targett::assigns_clause_struct_targett(
  ...
  messaget &log_param,
  const irep_idt &func_id)
  : assigns_clause_targett(Struct, pointer_for(obj_ptr), contr, log_param),
    main_struct_standin(typet())
{
  const symbolt &struct_sym =
    contract.get_namespace().lookup(to_tag_type(obj_ptr.type()));
  const symbolt &f_sym = contract.get_namespace().lookup(func_id);
  ...
  while(!component_members.empty())
  {
    ...
    // Declare a new symbol to stand in for the reference
    symbolt standin_symbol = contract.new_tmp_symbol(
      op_addr.type(), f_sym.location, func_id, f_sym.mode);
    ...
    {
      const symbolt &curr_struct_sym =
        contract.get_namespace().lookup(to_tag_type(curr_op.type()));
...
exprt assigns_clause_struct_targett::alias_expression(const exprt &ptr)
{
  ...
  for(symbol_exprt sym : local_standin_vars)
  {
    ...
    auto right_size =
      size_of_expr(standin_concrete_type, contract.get_namespace());
    if(!left_size.has_value())
    {
      log.error().source_location = ptr.source_location();
      log.error() << "unable to determine size of type (lhs): "
                  << type2c(ptr_concrete_type, contract.get_namespace())
                  << " and " << ptr_concrete_type.id() << messaget::eom;
      throw 0;
    }
    if(!right_size.has_value())
    {
      log.error().source_location = ptr.source_location();
      log.error() << "unable to determine size of type (rhs): "
                  << type2c(standin_concrete_type, contract.get_namespace())
                  << messaget::eom;
      throw 0;
    }
    if(
      std::strcmp(
        left_size.value().get(ID_value).c_str(),
        right_size.value().get(ID_value).c_str()) == 0)
    {
      exprt same_obj = same_object(ptr, sym);
      exprt same_offset = equal_exprt(pointer_offset(ptr), pointer_offset(sym));

      const exprt &compatible = and_exprt(same_obj, same_offset);
      if(first_iter)
      {
        running = compatible;
        first_iter = false;
  ...
  const assigns_clause_targett &called_target)
{
  ...
  else if(called_target.tgt_type == Struct)
  {
    ...
    exprt same_obj =
      same_object(this->main_struct_standin, struct_target.obj_pointer);
    // the size of the called struct should be less than or equal to that of the assignable target struct.
    exprt curr_size = get_size_or_throw(
      this->obj_pointer.type(), contract.get_namespace(), log);
    exprt curr_upper_offset =
      pointer_offset(plus_exprt(this->main_struct_standin, curr_size));
    exprt called_size = get_size_or_throw(
      struct_target.obj_pointer.type(), contract.get_namespace(), log);
    exprt called_upper_offset =
      pointer_offset(plus_exprt(struct_target.obj_pointer, called_size));

    exprt in_range_lower = binary_predicate_exprt(
      pointer_offset(struct_target.obj_pointer),
      ID_ge,
...
goto_programt
assigns_clause_struct_targett::havoc_code(source_locationt loc) const
{
  goto_programt assigns_havoc;
...
assigns_clause_array_targett::assigns_clause_array_targett(
  ...
  : assigns_clause_targett(
      Array,
      to_int_range_exprt(obj_ptr).op0(),
      contr,
      log_param),
    lower_offset_obj(),
    upper_offset_obj(),
    arr_standin_var(typet()),
    lower_offset_var(typet()),
    upper_offset_var(typet())
{
  const exprt &arr = to_int_range_exprt(obj_ptr).op0();
  const exprt &range = to_int_range_exprt(obj_ptr).op1();

  // If the range doesn't have operands, it is just a single value
  const exprt &lower_op =
    range.has_operands() ? to_int_range_exprt(range).op0() : range;
  const exprt &upper_op =
    range.has_operands() ? to_int_range_exprt(range).op1() : range;
  ...
  {
    int lowerbase =
      std::stoi(to_constant_expr(lower_op).get(ID_C_base).c_str(), nullptr, 10);
    lower_bound = std::stoi(
      to_constant_expr(lower_op).get_value().c_str(), nullptr, lowerbase);
    ...
    exprt lower_constant_size =
      get_size_or_throw(arr.type().subtype(), contract.get_namespace(), log);
    lower_offset_obj = typecast_exprt(
      mult_exprt(
    ...
    symbolt lower_standin_symbol = contract.new_tmp_symbol(
      lower_offset_obj.type(), f_sym.location, func_id, f_sym.mode);
  ...
  else
  {
    ...
    lower_offset_obj = typecast_exprt(
      mult_exprt(
        typecast_exprt(lower_op, unsigned_long_int_type()),
        lower_constant_size),
      signed_int_type());
  ...
  {
    int upperbase =
      std::stoi(to_constant_expr(upper_op).get(ID_C_base).c_str(), nullptr, 10);
    upper_bound = std::stoi(
      to_constant_expr(upper_op).get_value().c_str(), nullptr, upperbase);
    ...
    exprt upper_constant_size =
      get_size_or_throw(arr.type().subtype(), contract.get_namespace(), log);
    upper_offset_obj = typecast_exprt(
      mult_exprt(
    ...
    symbolt upper_standin_symbol = contract.new_tmp_symbol(
      upper_offset_obj.type(), f_sym.location, func_id, f_sym.mode);
  ...
  else
  {
    ...
    upper_offset_obj = typecast_exprt(
      mult_exprt(
        typecast_exprt(upper_op, unsigned_long_int_type()),
        upper_constant_size),
      signed_int_type());
...
goto_programt
assigns_clause_array_targett::havoc_code(source_locationt loc) const
{
  goto_programt assigns_havoc;
  ...
  exprt array_type_size = get_size_or_throw(
    obj_pointer.type().subtype(), contract.get_namespace(), log);
  ...
  for(int i = lower_bound; i <= upper_bound; ++i)
  {
    ...
    dereference_exprt array_deref(plus_exprt(
      obj_pointer, typecast_exprt(val_const, signed_long_int_type())));
...
exprt assigns_clause_array_targett::alias_expression(const exprt &ptr)
{
  ...
  exprt in_range_lower = binary_predicate_exprt(
    ptr_offset, ID_ge, typecast_exprt(lower_offset_var, ptr_offset.type()));
  exprt in_range_upper = binary_predicate_exprt(
    typecast_exprt(upper_offset_var, ptr_offset.type()), ID_ge, ptr_offset);
  exprt in_range = and_exprt(in_range_lower, in_range_upper);
  ...
  const assigns_clause_targett &called_target)
{
  ...
  else if(called_target.tgt_type == Array)
  {
    ...
    exprt same_obj =
      same_object(this->arr_standin_var, array_target.obj_pointer);
    exprt in_range_lower = binary_predicate_exprt(
      array_target.lower_offset_obj, ID_ge, this->lower_offset_var);
    exprt in_range_upper = binary_predicate_exprt(
      this->upper_offset_var, ID_ge, array_target.upper_offset_obj);
    exprt in_range = and_exprt(in_range_lower, in_range_upper);
    return and_exprt(same_obj, in_range);
  }
  else // Struct
...
assigns_clause_targett *assigns_clauset::add_target(exprt curr_op)
{
  ...
  {
    assigns_clause_array_targett *array_tgt =
      new assigns_clause_array_targett(curr_op, parent, log, func_id);
    targets.push_back(array_tgt);
    return array_tgt;
  }
  else if(curr_op.type().id() == ID_struct_tag)
  {
    assigns_clause_struct_targett *struct_tgt =
      new assigns_clause_struct_targett(curr_op, parent, log, func_id);
    targets.push_back(struct_tgt);
    return struct_tgt;
  }
  else
  {
    assigns_clause_scalar_targett *scalar_tgt =
      new assigns_clause_scalar_targett(curr_op, parent, log, func_id);
    ...
    for(assigns_clause_targett *target : targets)
    {
      for(symbol_exprt sym : target->temporary_declarations())
      {
        standin_declarations.add(
          goto_programt::make_decl(sym, sym.source_location()));
      }
    }
  }
