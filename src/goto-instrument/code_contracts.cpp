  ...
  goto_programt::targett target)
{
  ...
  if(assigns.is_not_nil())
  {
    ...
    goto_programt assigns_havoc = assigns_cause.havoc_code(function_symbol.location, func_id, function_symbol.mode);
  ...
  assigns_clauset &assigns_clause)
{
  ...
  alias_assertion.add(
    goto_programt::make_assertion(cast_alias, instruction_iterator->source_location));
  ...
  assigns_clauset &assigns_clause)
{
    ...
    == freely_assignable_symbols.end())
  {
    ...
    exprt alias_expr = assigns_clause.alias_expression(call.lhs());
      ...
      goto_programt alias_assertion;
    alias_assertion.add(
      goto_programt::make_assertion(alias_expr, instruction_iterator->source_location));
    program.insert_before_swap(instruction_iterator, alias_assertion);
  ...
  if(call.function().id() == ID_symbol)
  {
    ...
    else // Called function has assigns clause
    {
      ...
      goto_programt alias_assertion;
      alias_assertion.add(
              goto_programt::make_assertion(compat, instruction_iterator->source_location));
      program.insert_before_swap(instruction_iterator, alias_assertion);
      ++instruction_iterator;
    }
  }
...
bool code_contractst::add_pointer_checks(const std::string &function_name)
{
  ...
  goto_programt mark_dead = assigns.dead_stmts(function_symbol.location, function_name, function_symbol.mode);
      ...
      std::advance(instruction_iterator, 1))
  {
    if(instruction_iterator->is_decl())
    {
      ...
      assigns_clause_targett *new_target = assigns.add_target(instruction_iterator->get_decl().symbol());
      goto_programt &pointer_capture = new_target->get_init_block();

      lines_to_iterate = pointer_capture.instructions.size();
      for(auto in : pointer_capture.instructions)
      {
        program.insert_after(instruction_iterator, in);
        std::advance(instruction_iterator, 1);
      }
    }
    else if(instruction_iterator->is_assign())
    {
      instrument_assn_statement(function_name, instruction_iterator, program, assigns_expr, freely_assignable_symbols, assigns);
    }
    else if(instruction_iterator->is_function_call())
    {
      instrument_call_statement(instruction_iterator, program, assigns_expr, function_id, freely_assignable_symbols, assigns);
    }
  }

  // walk the iterator back to the last statement
  while(!instruction_iterator->is_end_function()) {std::advance(instruction_iterator, -1);}

  // Make sure the temporary symbols are marked dead
  lines_to_iterate = mark_dead.instructions.size();
  program.insert_before_swap(instruction_iterator, mark_dead);
...
assigns_clause_array_targett::assigns_clause_array_targett(const exprt &obj_ptr, const code_contractst &contr, messaget &log_param, const irep_idt &func_id) :
        assigns_clause_targett(Array, obj_ptr.operands()[0], contr, log_param),lower_offset_obj(), upper_offset_obj(), arr_standin_var(typet()), lower_offset_var(typet()), upper_offset_var(typet())
{
  const exprt &arr = obj_ptr.operands()[0];
  const exprt &range = obj_ptr.operands()[1];

  // If the range doesn't have operands, it is just a single value
  const exprt &lower_op = range.has_operands() ? range.operands()[0] : range;
  const exprt &upper_op = range.has_operands() ? range.operands()[1] : range;
