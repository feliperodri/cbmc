...
{
  __CPROVER_assert(
    0, "__CPROVER_is_fresh called outside of contract function.");
  return 0;
}
...
{
  *elem = malloc(size);
  if(!*elem || __CPROVER_FUN_NAME_memory_map[(unsigned long)*elem])
    return 0;
...
{
  _Bool ok =
    (!__CPROVER_FUN_NAME_memory_map[(unsigned long)elem] &&
...
_Bool __CPROVER_FUN_NAME_call_requires_is_fresh(
  void *elem,
  __CPROVER_size_t size)
{
  _Bool r_ok = __CPROVER_r_ok(elem, size);
  if(__CPROVER_FUN_NAME_memory_map[(unsigned long)elem] || !r_ok)
    return 0;
  __CPROVER_FUN_NAME_memory_map[(unsigned long)elem] = 1;
  return 1;
}
...
_Bool __CPROVER_FUN_NAME_call_ensures_is_fresh(
  void **elem,
  __CPROVER_size_t size)
{
  *elem = malloc(size);
